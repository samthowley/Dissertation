...
master <- int.ext %>%
  select(Date, ID, avg.external, Basin, day, NEP_corrected) %>%
  ...
  fill(avg.external, Basin, avg.external, K600, NEP_corrected, .direction = 'down')%>%
  drop_na()


# corr_mat <- cor(master%>%select(-Date, -ID, -AREA, -avg.external, -avg.external, -Basin, -day),
...
for (i in seq_len(nrow(cv_results))) {
  ...
  cv_results$RMSE[i] <- sqrt(mean((preds - test_fold$avg.external)^2))
}
...
plot(
  rf_test$avg.external,
  pred,
  xlab = "external CO₂ Flux",
  ylab = "external CO₂ Flux",
  pch = 16
)
abline(0, 1, col = "red", lwd = 2)

rmse <- sqrt(mean((pred - rf_test$avg.external)^2))
r2 <- cor(pred, rf_test$avg.external)^2
...
ggplot(pred_df, aes(x = obs, y = pred)) +
  ...
  theme_bw() +
  labs(
    x = "Observed external CO\u2082 flux",
    y = "Predicted external CO\u2082 flux",
  ...
  for (r in seq_len(repeats)) {
    ...
    for (j in seq_along(mtry_vals)) {
      ...
      for (f in 1:k) {
        ...
        rmse_fold[f] <- sqrt(mean((preds - te$avg.external)^2, na.rm = TRUE))
      }
    ...
    rmse <- sqrt(mean((pred - test$avg.external)^2, na.rm = TRUE))

    # guard against NA when variance is zero
    r2 <- suppressWarnings(cor(pred, test$avg.external, use = "complete.obs")^2)
    if (is.na(r2)) r2 <- 0
...
# --- NRMSE FIX: normalize by SD of the RESPONSE (avg.external), by site ---
sd_by_site <- rf.df %>%
  group_by(ID) %>%
  summarise(sd_y = sd(avg.external, na.rm = TRUE), .groups = "drop")
...
ggplot(plot_df, aes(x = ID, y = rel_imp, fill = variable)) +
  ...
  ggtitle("Relative Importance of Factors Influencing the external Pathway")
