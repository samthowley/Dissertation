...
lmm_final <- lmer(
  avg.external ~ Q + Temp_DO + depth + watershed.inundation + SpC +
    (1 | ID),
  data = lm_df,
  REML = FALSE)
...
get_site_R2 <- function(df) {
  ...
  mod <- lm(avg.external ~ Temp_DO + Q + depth + watershed.inundation + SpC, data = df)
...
get_site_betas <- function(df) {
  ...
  df_std <- df %>%
    mutate(
      avg.external = scale(avg.external)[,1],
      across(all_of(preds), ~ scale(.x)[,1])
    )

  # fit per-site model
  mod <- lm(avg.external ~ Temp_DO + Q + depth + watershed.inundation + SpC, data = df_std)
