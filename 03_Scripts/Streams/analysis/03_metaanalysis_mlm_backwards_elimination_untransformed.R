...
# ── UNTRANSFORMED variant ────────────────────────────────────────────────────
# Exploratory sibling of 03_metaanalysis_mlm_backwards_elimination.R -- same
# predictors/weighting/random effects/selection rule, but fit on the raw
# response scale (Internal_Pct_of_Flux, Internal_Pathway_gCm2day,
# External_Pathway_gCm2day) instead of the logit/asinh transforms Script 2
# (prelim_checks) found necessary for normality/boundary behavior. Not part
# of the modeling pipeline -- a side-by-side curiosity check, not a
# replacement. Writes to separate _untransformed output files so it never
# clobbers the real pipeline's results.
...
response_specs <- list(
  list(key = "pct_internal",  label = "Internal % of Total CO2 flux (untransformed)",       response = "Internal_Pct_of_Flux"),
  list(key = "internal_flux", label = "Internal pathway flux (C g/m²/day, untransformed)",  response = "Internal_Pathway_gCm2day"),
  list(key = "external_flux", label = "External pathway flux (C g/m²/day, untransformed)",  response = "External_Pathway_gCm2day")
)
...
out_path <- "04_Output/metaanalysis_mlm_backwards_elimination_summary_untransformed.csv"
write_csv(final_summary_all, out_path)

tables_out_path <- "05_Figures/Table_metaanalysis_mlm_backwards_elimination_untransformed.docx"
if (length(selected_specs) > 0) {
