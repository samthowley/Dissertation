...
CO2<-chem%>%
  select(Date, ID, CO2, pH, SpC, Temp_DO, depth, Q, DO)%>%
...
for (i in seq_len(nrow(cv_results))) {
  ...
  cv_results$RMSE[i] <- sqrt(mean((preds - test_fold$CO2)^2))
}
...
ggplot(pred_df, aes(x = obs, y = pred)) +
  ...
  theme_bw() +
  labs(
    x = "Observed internal CO2",
    y = "Predicted internal CO2",
  ...
  for (r in seq_len(repeats)) {
    ...
    for (j in seq_along(mtry_vals)) {
      ...
      for (f in 1:k) {
        ...
        rmse_fold[f] <- sqrt(mean((preds - te$CO2)^2, na.rm = TRUE))
      }
    ...
    rmse <- sqrt(mean((pred - test$CO2)^2, na.rm = TRUE))

    # guard against NA when variance is zero
    r2 <- suppressWarnings(cor(pred, test$CO2, use = "complete.obs")^2)
    if (is.na(r2)) r2 <- 0
...
# --- NRMSE FIX: normalize by SD of the RESPONSE (CO2), by site ---
sd_by_site <- rf.df %>%
  group_by(ID) %>%
  summarise(sd_y = sd(CO2, na.rm = TRUE), .groups = "drop")
...
cont_vars <- c("Q", "depth", 'DO', "Temp_DO", "watershed.inundation", "SpC")
...
lmm_final <- lmer(
  CO2 ~ Q + Temp_DO + depth + watershed.inundation + SpC +DO+
    (1 | ID),
  data = lm_df,
  REML = FALSE)
summary(lmm_final)
...
plot_df <- tibble(
  variable = c("Temp_DO", "Q", "depth", "watershed.inundation", "SpC", "DO"),
  # unique (semi-partial) R2: only available for DO and depth from your partR2 run
  unique_R2 = c(NA, NA, 0.1315, NA, NA, 0.3080),
  # inclusive R2: available for all predictors from the summary
  inclusive_R2 = c(0.0036, 0.0018, 0.0773, 0.0257, 0.0223, 0.3642)
) %>%
  mutate(
    shared_R2 = if_else(is.na(unique_R2), NA_real_, pmax(inclusive_R2 - unique_R2, 0))
  ) %>%
  pivot_longer(
...
ggplot(plot_df, aes(x = variable, y = rel_imp, fill = component)) +
  ...
  ggtitle("Predictive Power of Model Variables",
          subtitle = "Bars show total contribution; shading indicates shared (collinear) effects")
...
get_site_R2 <- function(df) {
  ...
  mod <- lm(CO2 ~ Temp_DO + Q + depth + watershed.inundation + SpC+ DO, data = df)
  ...
  preds <- c("Temp_DO", "Q", "depth", "watershed.inundation", "SpC", "DO")
...
get_site_betas <- function(df) {
  ...
  # fit per-site model
  mod <- lm(
    CO2 ~ Temp_DO + Q + depth + watershed.inundation + SpC + DO,
    data = df_std
  )
...
ggplot(site_beta_df%>%filter(variable %in% c('DO', 'Temp_DO')), aes(x = variable, y = beta, fill = beta > 0)) +
...
#figures#########
left_join(int.ext, CO2)%>%left_join(temperature)%>%
  ggplot(aes(x=CO2, y=avg.external))+
  geom_point(aes(colour = Temp_PT))+
  #lm.common+
  facet_wrap(~ID, scales='free')
test<-left_join(int.ext, CO2)%>%left_join(temperature)
