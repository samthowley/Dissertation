...
ggplot(int.ext,
       aes(x = Q, y = CO2, group = ID, color = ID)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = lm, se = FALSE) +
  scale_y_log10() + scale_x_log10() +
  annotate("text", x = Inf, y = Inf, label = stats_label,
           hjust = 1.1, vjust = 1.5, size = 3.5, fontface = "italic") +
  ggtitle(expression('CO2'~'Pathway'~'Responses'~'to'~'Discharge')) +
  ylab(expression(CO[2]~'g'/m^2/'day')) +
  xlab(expression('Discharge'~'L'~s^-1)) +
  theme_classic()
...
ggplot(int.ext,
       aes(x = Q, y = CO2_flux, group = ID, color = ID)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = lm, se = FALSE) +
  scale_y_log10() + scale_x_log10() +
  annotate("text", x = Inf, y = Inf, label = stats_label,
           hjust = 1.1, vjust = 1.5, size = 3.5, fontface = "italic") +
  ggtitle(expression('CO2_flux'~'Pathway'~'Responses'~'to'~'Discharge')) +
  ylab(expression(CO[2]~'g'/m^2/'day')) +
  xlab(expression('Discharge'~'L'~s^-1)) +
  theme_classic()
