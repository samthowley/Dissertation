# ══════════════════════════════════════════════════════════════════════════════
# LMM Figure Options: Full vs. Drop-T Model Comparison
# Focus: β_T estimate stability AND R² degradation when T is removed
...
# Drop T — pathway-specific T estimates with CrI (dropped_from == own pathway)
noT_T <- bind_rows(
  dropT_raw %>% filter(dropped_from == "lint", pathway == "lint", indep == "lQ"),
  dropT_raw %>% filter(dropped_from == "lext", pathway == "lext", indep == "lQ")
) %>%
  transmute(
    ...
    R2,
    sigma,
    model       = "Drop T (pathway)"
  )

# Drop T — both pathways simultaneously (dropped_from == "both")
bothQ_T <- bind_rows(
  dropT_raw %>% filter(dropped_from == "both", pathway == "lint", indep == "lQ"),
  dropT_raw %>% filter(dropped_from == "both", pathway == "lext", indep == "lQ")
) %>%
  transmute(
    site        = factor(as.character(site), levels = SITE_LEVELS),
    pathway,
...
comb <- bind_rows(full_T, noT_T, bothQ_T) %>%
  mutate(model = factor(model,
                        levels = c("Full (Q + T)", "Drop T (pathway)", "Drop T (both)")))
...
wide <- full_T %>%
  ...
  left_join(
    noT_T %>%
      select(-model) %>%
      rename_with(~ paste0("noT_", .), c(estimate, lo, hi, R2, sigma)),
  ...
  mutate(
    delta_est_noT  = noT_estimate  - full_estimate,
    delta_est_both = both_estimate - full_estimate,
    delta_R2_noT   = noT_R2        - full_R2,    # negative = fit loss
    delta_R2_both  = both_R2       - full_R2,
    delta_sig_noT  = noT_sigma     - full_sigma,
...
A_colors <- c("Full (Q + T)"     = COL_FULL,
              ...
              "Drop T (both)"    = COL_BOTH)

A_shapes <- c("Full (Q + T)"     = 16,   # filled circle
              "Drop T (pathway)" = 15,   # open circle
              "Drop T (both)"    = 17)   # filled triangle

A_sizes  <- c("Full (Q + T)"     = SZ_PT + 0.5,
              "Drop T (pathway)" = SZ_PT + 0.5,
              "Drop T (both)"    = SZ_PT)
...
(pA_left <- comb %>%
  ...
  geom_segment(
    data = wide,
    aes(x = full_estimate, xend = noT_estimate, y = site, yend = site),
    inherit.aes = FALSE,
...
wide_dR2 <- wide %>%
  select(site, pathway_lab, delta_R2_noT, delta_R2_both) %>%
  pivot_longer(
    cols      = c(delta_R2_noT, delta_R2_both),
    names_to  = "condition",
    values_to = "delta_R2"
  ) %>%
  mutate(
    condition = case_match(condition,
                           "delta_R2_noT"  ~ "Drop T (pathway)",
                           "delta_R2_both" ~ "Drop T (both)")
  )
...
(pA_right <- wide_dR2 %>%
  ...
  scale_color_manual(
    values = c("Drop T (pathway)" = COL_DROP, "Drop T (both)" = COL_BOTH)
  ) +
  scale_shape_manual(
    values = c("Drop T (pathway)" = 15, "Drop T (both)" = 17)
  ) +
