...
plot_grid(
  ...
  p_flux_vs_temp+
    ggtitle(expression(CO[2]~'Pathway'~'Response'~'to'~bold('Spatial')~'Differences in Stream Temperature'))
),
...
plot_grid(
  temp_Q+
    theme(legend.position = "none"), 
  p_flux_vs_Q+
    ggtitle(expression(CO[2]~'Pathway'~'Response'~'to'~bold('Spatial')~'Differences in Discharge'))
),
